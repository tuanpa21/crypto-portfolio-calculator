...
const CRYPTO_COMPARE_API = (token: string) =>
...
async function getPortfolioValue() {
  console.clear();
  console.log(
    chalk.green("🚀 Starting Crypto Portfolio Value Calculator...\n")
  );
  ...
  try {
    ...
    const tableData = Array.from(tokenValues.entries()).map(
      ([token, value]): TableData => {
        // Validate that the amount of tokens is not negative
        if (value < 0) {
          console.warn(`⚠️ Insufficient balance for ${token} token.`);
        }
        return {
          Token: token,
    ...
    console.log(
      chalk.blue(
        `✅ Data collection and processing completed in ${duration} seconds.\n`
      )
    );
    console.log(
      chalk.yellow("💰 Your latest portfolio value per token in USD:\n")
    );
...
function updateBalance(data: Transaction, balances: Map<string, number>) {
  const { token, amount, transaction_type } = data; // Destructure the data

  if (!balances.has(token)) {
    balances.set(token, 0); // Initialize the balance to 0 if it doesn't exist
  }

  const currentBalance = balances.get(token) || 0; // Get the current balance
